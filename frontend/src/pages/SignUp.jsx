...
const Signup = () => {
  ...
  const pfpUpload = async (pic) => {
    ...
    if (
      ...
      data.append("cloud_name", "dk13xx91w");
      data.append('folder', 'social-media-project-data');


      // Send a POST request to Cloudinary API to upload the image
      fetch("https://api.cloudinary.com/v1_1/dk13xx91w/image/upload", {
        method: "post",
        body: data,
      })
        .then((res) => res.json())
  ...
  return (
    <div className="h-screen w-screen bg-zinc-900 bg-cover bg-top flex flex-col items-center justify-center">
      <AuthContainer>
        {/* Render the Logo component */}
              <h1 className="text-white font-bold text-xl">Socialify</h1>
